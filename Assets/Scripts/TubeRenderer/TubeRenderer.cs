...
public class TubeRenderer : MonoBehaviour
{
	...
	void OnDrawGizmos()
	{
		...
		if( _showMeshGizmos )
		{
			for( int v=0; v<_vertices.Length; v++ )
			{
				...
				Gizmos.DrawLine( _vertices[v], _vertices[v] + _normals[v] * meshGizmoLength );
				
				// Tangents.
				if( calculateTangents && _tangents.Length != 0 ){
					if( _tangents[v].w == -1 ) Gizmos.color = new Color( 1, 0, 1, 0.5f );
					else if( _tangents[v].w == 1 ) Gizmos.color = new Color( 1, 1, 0, 0.5f );
					else Gizmos.color = Color.white;
					Gizmos.DrawLine( _vertices[v], _vertices[v] + new Vector3( _tangents[v].x, _tangents[v].y, _tangents[v].z ) * meshGizmoLength );
				}
			}
		}
		...
		if( _showRotationGizmos )
		{
			...
			float dotSize = meshGizmoLength * 0.2f;
			for( int p = 0; p < _points.Length; p++ ){
				float offset = ( usingRadiuses ? _radiuses[p] : _radius ) + meshGizmoLength * 4;
	...
	void ReDraw()
	{
		...
		if( _caps == CapMode.Both || _caps == CapMode.Begin ){
			Vector3 normal = _rotations[0] * Vector3.back * invertSign;
			Vector4 tangent = _rotations[0] * Vector3.right;
			tangent.w = -1;
	...
	void ReDrawHardNormals()
	{
		...
		for( int p=_points.Length-2; p>0; p-- ){ // don't copy vertices from first circle
			for( int e=_edgeCount-1; e>=0; e-- ){
				...
				Vector3 normal = Vector3.Cross( _vertices[lv+quad[3]] - _vertices[lv], _vertices[lv+quad[1]] - _vertices[lv] );
				normal.Normalize();
				for( int q=0; q<4; q++ ){
					_vertices[v] = _vertices[lv+quad[q]];
		...
		for( int e=_edgeCount-1; e>=0; e-- ){
			v = e * 4;
			Vector3 normal = Vector3.Cross( verticesTemp[e+quad[2]] - verticesTemp[e], verticesTemp[e+quad[1]] - verticesTemp[e] );
			normal.Normalize();
			for( int q=0; q<4; q++ ){
				_normals[v].Set( normal.x, normal.y, normal.z );
				_vertices[v] = verticesTemp[e+quad[q]];
				v++;
			}
		}
	...
	void ReDrawHardNormalEdges()
	{
		...
		for( int p=1; p>=0; p-- ){
			for( int e=_edgeCount-1; e>=0; e-- ){
				...
				_normals[v+1] = _normals[v];
				_vertices[v] = verticesTemp[lv];
				_vertices[v+1] = verticesTemp[lv+1];
			}
		}
	...
	void UpdateRotations()
	{
        ...
        Vector3 up;
        bool firstFrame = _pastBeginUp == Vector3.zero;
        if( firstFrame ){
            up = Vector3.Dot( _directions[0], Vector3.right ) <= 0 ? Vector3.right : Vector3.up;
        } else {
            up = _pastBeginUp;
        }
	...
	void UpdateSteepnessAngles()
	{
		...
		for( int p=0; p<_points.Length-1; p++ ){
			...
			if( avgRadiusDiff == 0 ) _steepnessAngles[p] = 0;
	...
	void UpdateColors()
	{
		if( _pointColors.Length == 0 ){
			...
			if( _colors32 != null ) _colors32 = null;
			_dirtyColors = false;
			return;
		}
